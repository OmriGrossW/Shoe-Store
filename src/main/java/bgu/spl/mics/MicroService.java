...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap<Message, Callback> onCompleteMap = new ConcurrentHashMap<>();
    ...
    protected final <R extends Request> void subscribeRequest(Class<R> type, Callback<R> callback) {
    	MessageBusImpl.getMessageBus().subscribeRequest(type, this);
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
    	MessageBusImpl.getMessageBus().subscribeBroadcast(type, this);
    	callbacksMap.put(type, callback);
    }
    ...
    protected final <T> boolean sendRequest(Request<T> r, Callback<T> onComplete) {
       if (MessageBusImpl.getMessageBus().sendRequest(r, this)){
    ...
    protected final void sendBroadcast(Broadcast b) {
    	MessageBusImpl.getMessageBus().sendBroadcast(b);    
    	}
    ...
    protected final <T> void complete(Request<T> r, T result) {
    	MessageBusImpl.getMessageBus().complete(r, result);
    }
     ...
     * each micro-service has a unique initialize function, 
     * after the initialization has finished it waits for messages and handles them until it terminates (gracefully of course ;) ).
     */
    @Override
    public final void run() {
        ...
        while (!terminated) {
        	try {
        		Message m= MessageBusImpl.getMessageBus().awaitMessage(this);
				if(m.getClass()==RequestCompleted.class){
					onCompleteMap.get(((RequestCompleted)m).getCompletedRequest()).call(((RequestCompleted)m).getResult());
				}
				else{
